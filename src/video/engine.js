...
function createInput(file) { return new Input({ source: new BlobSource(file), formats: ALL_FORMATS }); }
...
async function metadataOf(input, track, { includeStats = false } = {}) {
  const [width, height, firstTimestamp, codec, durationMeta] = await Promise.all([
    track.getDisplayWidth(), track.getDisplayHeight(), track.getFirstTimestamp().catch(() => 0),
    track.getCodec().catch(() => null), input.getDurationFromMetadata([track], { skipLiveWait: true }).catch(() => null)
  ]);
  const duration = Number.isFinite(durationMeta) && durationMeta > 0 ? durationMeta : await track.computeDuration({ skipLiveWait: true }).catch(() => null);
  const packetStats = includeStats ? await track.computePacketStats(90, { skipLiveWait: true }).catch(() => null) : null;
  const frameRate = Number.isFinite(packetStats?.averagePacketRate) && packetStats.averagePacketRate > 0 ? packetStats.averagePacketRate : 30;
  return {
    width, height, firstTimestamp: Number.isFinite(firstTimestamp) ? firstTimestamp : 0, codec,
    duration: Number.isFinite(duration) ? duration : null, frameRate,
...
function cropImageData(imageData, position) {
  ...
  const x0 = Math.round(position.x), y0 = Math.round(position.y);
  const data = new Uint8ClampedArray(width * height * 4);
  for (let y = 0; y < height; y++) {
    const sourceStart = ((y0 + y) * imageData.width + x0) * 4;
    data.set(imageData.data.subarray(sourceStart, sourceStart + width * 4), y * width * 4);
  }
...
export function repairPaddedRegion(paddedOriginal, inner, alphaMap, gain, edgePolish, history = [], allowMaskedDonors = false) {
  ...
  const paddedAlpha = embedAlphaMap(alphaMap, inner.width, inner.height, paddedOriginal.width, paddedOriginal.height, inner.offsetX, inner.offsetY);
...
export async function inspectVideo(file, options = {}) {
  ...
  try {
    ...
    const frames = await sampleFrames(track, metadata, options.sampleCount, options.onProgress, options.shouldCancel, options.scanFraction ?? 1);
    ...
    const detection = await detectVideoWatermarkFromFrames({ frames, width: metadata.width, height: metadata.height, minConfidence: options.minConfidence ?? 0.12, edgePolish: options.edgePolish ?? 0.35 });
...
async function prepareAudioCopy(input, output, format, startTimestamp) {
  ...
  if (!codec || !format.getSupportedAudioCodecs().includes(codec)) return { source: null, track, result: { copied: false, reason: 'unsupported-audio-codec', codec } };
...
function normalizeRegion(region) {
  ...
  return { x: Math.round(region.x), y: Math.round(region.y), width: size, height: size };
}
...
export async function cleanVideo(file, options = {}) {
  ...
  if (!manual && !detectedRegion) {
    ...
    if (!analysis.internalDetection.detected && !options.forceCleanup) throw new Error(`Watermark confidence ${analysis.internalDetection.confidence.toFixed(3)} is below threshold. Enable force cleanup or use manual mode.`);
  }
  const input = createInput(file);
  const track = await input.getPrimaryVideoTrack();
  if (!track) { input.dispose(); throw new Error('No video track found'); }
  ...
  const source = new CanvasSource(canvas, {
    codec: 'avc', bitrate: Number.isFinite(options.bitrate) ? options.bitrate : 12_000_000, alpha: 'discard',
    keyFrameInterval: 2, latencyMode: 'quality', bitrateMode: 'constant', hardwareAcceleration: 'no-preference', contentHint: 'detail',
    onEncodedPacket(_packet, meta) { if (meta?.decoderConfig) meta.decoderConfig.colorSpace = { ...DEFAULT_COLOR_SPACE }; }
  });
  ...
  try {
    ...
    const sink = new VideoSampleSink(track);
    for await (const sample of sink.samples()) {
      const rawTimestamp = sample.timestamp, rawDuration = sample.duration;
      try {
        ...
        } else { skippedFrames++; previousTemporal = null; history = []; }
      } finally { sample.close(); }
      ...
      options.onProgress?.({ phase: 'export', status: `Exporting frame ${processedFrames}${metadata.frameCountEstimate ? `/${metadata.frameCountEstimate}` : ''}`, progress: 0.65 + 0.34 * Math.min(1, processedFrames / estimate) });
    }
    source.close();
