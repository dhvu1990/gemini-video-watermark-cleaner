...
function renderDetection(result, { source = 'full' } = {}) {
  ...
  if (d?.detected) { els.analysisSummary.textContent = `Auto-detected: Gemini Veo — ${label} (${match}% match)`; const bg = adaptiveBackground?.mode === 'empty-hard-rebuild' ? ' Safe empty-zone hard suppression enabled.' : (adaptiveBackground?.mode === 'smooth-rebuild' ? ' Smooth background rebuild enabled.' : ' Structured-background fallback.'); els.previewNote.textContent = (source === 'quick' ? `Quick scan accepted with ${QUICK_SAMPLE_COUNT} sampled frames.` : 'Full scan confirmed the watermark across multiple frames.') + bg; setStatus(`Detected: ${d.candidateId} confidence ${confidence.toFixed(3)}`, 1); }
